...
/**
 * This is the model class for collection "cronJob".
 * @author Federico Nicolás Motta <fedemotta@gmail.com>.Modified by Br Saket for mongodb.
...
class CronJob extends ActiveRecord
{
            ...
            'timestamp' => 
                [
                    ...
                    [
                        ...
                        ActiveRecord::EVENT_BEFORE_UPDATE => ['ended_at'],
                    ]
                ],
                [
                    'class' => 'common\components\autonumber\Behavior',
                    'attribute' => 'id_cron_job', // required
    ...
    public static function run($controller, $action, $limit, $max)
    {
        ...
        if ($current->save()):
           ...
           Console::stdout("*** running ".$current->controller."/".$current->action."\n");
            return $current;
        
        else:
            /*Console::stdout(Console::ansiFormat("*** failed to run ".$current->controller."/".$current->action."\n", [Console::FG_RED]));*/
            Console::stdout("*** failed to run ".$current->controller."/".$current->action."\n");
            return false;
        endif;   
    }
    ...
    public function finish()
    {
        ...
        if ($this->save()):
           /* Console::stdout(Console::ansiFormat("*** finished ".$this->controller."/".$this->action." (time: " . sprintf("%.3f", $this->last_execution_time) . "s)\n\n", [Console::FG_GREEN]));*/
            Console::stdout("*** finished ".$this->controller."/".$this->action." (time: " . sprintf("%.3f", $this->last_execution_time) . "s)\n\n");
            return true;
        else:
           /* Console::stdout(Console::ansiFormat("*** failed to finish ".$this->controller."/".$this->action." (time: " . sprintf("%.3f", $this->last_execution_time) . "s)\n\n", [Console::FG_RED]));*/
            ...
            Console::stdout("*** failed to finish ".$this->controller."/".$this->action." (time: " . sprintf("%.3f", $this->last_execution_time) . "s)\n\n");
            return false;
        endif;
    }
